...
replicas = [
    ...
    'ты проходишь мимо, но тут тебя подозвал к себе какой-то неперсточник-циган. \nсыграть с ним?',
...
my_proxies = list(tariff_broadband_df.ip)

while True:
    try:
        my_proxy = my_proxies[index_of_ip]
        apihelper.proxy = {'https':'socks5h://' + my_proxy}
        ...
        print('current ip = {}'.format(my_proxy))
        ...
        if attempt_count > 5:
            attempt_count = 0
            index_of_ip += 1
            if index_of_ip >= len(my_proxies):
                index_of_ip = 0
        time.sleep(1)
        print ("Internet error! ip_index={} attempt_count={}".format(index_of_ip, attempt_count))
